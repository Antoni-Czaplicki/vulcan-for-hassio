...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    hass.data[DOMAIN]["lessons"] = await get_lesson_info(student_id=config_entry.data.get("student_id"))
    hass.data[DOMAIN]["lessons_t"] = await get_lesson_info(student_id=config_entry.data.get("student_id"), date_from=datetime.date.today() + timedelta(days=1))
    ...
    hass.data[DOMAIN]["exam"] = await get_next_exam(
        config_entry.data.get("student_id")
    )
    hass.data[DOMAIN]["student_name"] = "Test"
...
class VulcanLessonEntity(VulcanEntity):
    ...
    async def async_update(self):
        self.lesson_data = await get_lesson_info(student_id=self.student_id,date_from=self.num_tomorrow)
